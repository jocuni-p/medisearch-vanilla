...
export async function fetchMedication(nregistro) {
    ...
    if (!response.ok) {
        throw new Error(`El fetch al endpoint ${url} ha fallado con un código ${response.status}`); // Caerá en el catch de detail.controller o favorites.controller (según quien la llame)
    }
